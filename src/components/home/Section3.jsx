export default function Section3() {
  return (
    <>
      <div className="w-[100%] h-[100%] bg-[#222] text-white">
        <div data-aos="fade-right" className="p-[8rem] flex flex-col">
          <span className="text-[4.5rem] font-black ">
            디자이너의,
            <br /> Happy 😀 Freedom 😎 Help 🥹 위해,
            <br /> 브로커가 만났습니다 !<br /> 세상 아름다운 조합, 커네팅
          </span>
          <span
            data-aos="fade-right"
            className="text-[1.5rem] w-[150px] mt-[10rem] rounded-full px-[.5rem] py-[.5rem] text-center border border-white border-solid cursor-pointer"
          >
            구경하기
          </span>
        </div>
      </div>
      <div className="z-[10000]  mb-[3rem] flex items-center w-[110%] relative left-[-5%] h-[85px] bg-orange-500 rotate-[350deg] text-white text-[1.5rem] whitespace-nowrap drop-shadow-md">
        <span className="Section3_animation-move-left">
          전 세계 유저가 당신의 디자인에 관심을 가질 수 있도록 기회를 제공해보세요. 전 세계 유저가 당신의 디자인에
          관심을 가질 수 있도록 기회를 제공해보세요. 전 세계 유저가 당신의 디자인에 관심을 가질 수 있도록 기회를
          제공해보세요.
        </span>
      </div>
      <div className="flex items-center w-[110%] relative left-[-5%] h-[85px] bg-white  text-black text-[1.5rem] whitespace-nowrap drop-shadow-md">
        <span className="Section3_animation-move-Right">
          Happy 😀 Freedom 😎 Help Happy 😀 Freedom 😎 Help Happy 😀 Freedom 😎 Help Happy 😀 Freedom 😎 Help Happy 😀
          Freedom 😎 Help Happy 😀 Freedom 😎 Help Happy 😀 Freedom 😎 Help Happy 😀 Freedom 😎 Help Happy 😀 Freedom 😎
          Help
        </span>
      </div>
      <div className="z-[10000] flex items-center w-[110%] relative left-[-5%] h-[85px] bg-orange-500 rotate-[-3deg] text-white text-[1.5rem] whitespace-nowrap drop-shadow-md">
        <span className="Section3_animation-move-left">
          즐거운 대화 √ 디자인 만족 즐거운 대화 √ 디자인 만족 즐거운 대화 √ 디자인 만족 즐거운 대화 √ 디자인 만족 즐거운
          대화 √ 디자인 만족 즐거운 대화 √ 디자인 만족 즐거운 대화 √ 디자인 만족
        </span>
      </div>
    </>
  );
}
